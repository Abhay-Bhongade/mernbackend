...
const signup = async (req, res, next) => {
  console.log("signupdata in req.body", req.body);
  ...
  try {
    ...
    return next(
      new HttpError("Something went wrong.could not find Teacher", 500)
    );
  }
  if (existingUser) {
    return next(
      new HttpError(
        "Email is already registered.please use diffrent email",
        401
      )
    );
  }
  ...
  try {
    ...
    const error = new HttpError(
      "Could not create Teacher,please try again.",
      500
    );
    return next(error);
  }
  ...
  try {
    ...
    token = jwt.sign({ email: email, role: "user" }, process.env.JWT_KEY, {
      expiresIn: "5d",
    });
  } catch (err) {
  ...
  res.status(201).json({
    ...
    data: encrypt({ email: email, role: "user", token: token }),
  });
};
...
const login = async (req, res, next) => {
  ...
  if (!error.isEmpty()) {
    return next(
      new HttpError("Invalid input enter.please enter valid input.", 401)
    );
  }
  ...
  let identifiedUser;
  try {
    vendorUser = await getOneVendor({
      $or: [
        { email: email, disable: false },
        { userName: email, disable: false },
      ],
    });
    normalUser = await getOne({
      $or: [
        { email: email, disable: false },
        { userName: email, disable: false },
      ],
  ...
  if (!identifiedUser) {
    const error = new HttpError(
      "User is not registered or enter wrong email,or sign up first.",
      401
    );
    return next(error);
  }
  ...
  res.status(200).json({
    message: "User/Vendor Login successfully",
    data: encrypt({
      userId: identifiedUser.id,
      email: identifiedUser.email,
      role: identifiedUser.role,
      ...(identifiedUser.role === "vendor" && {
        roleType: identifiedUser.roleType,
      }),
      token: token,
    }),
  });
